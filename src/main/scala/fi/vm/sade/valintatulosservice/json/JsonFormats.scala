...
import fi.vm.sade.valintatulosservice.{VastaanottotilaSerializer, HakijanVastaanottoActionSerializer}
...
object JsonFormats {
  ...
  val customSerializers = List(new LanguageMapSerializer()) ++ enumSerializers ++ List(new EnsikertalaisuusSerializer, new HakijanVastaanottoActionSerializer, new VastaanottotilaSerializer)
