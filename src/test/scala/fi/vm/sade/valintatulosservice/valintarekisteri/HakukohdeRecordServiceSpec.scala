...
class HakukohdeRecordServiceSpec extends Specification with MockitoMatchers with MockitoStubs with CalledMatchers {
  ...
  trait HakukohdeRecordServiceWithMocks extends Mockito with Scope with MustThrownExpectations {
    ...
    val luonnosKoulutus = Koulutus("1.2.246.562.17.53513994722", Kausi("2017S"), "LUONNOS")
    val julkaistuKoulutus = Koulutus("1.2.246.562.17.42423443434", Kausi("2016S"), "JULKAISTU")
    ...
    val hakuFromTarjonta: Haku = Haku(hakuOid, korkeakoulu = true, yhteishaku = true, varsinainenhaku = true,
      ...
      YhdenPaikanSaanto(voimassa = true, "Korkeakoulutus ilman kohdejoukon tarkennetta"))

    val hakukohdeRecordService = new HakukohdeRecordService(hakuService, hakukohdeRepository)
  }
}
