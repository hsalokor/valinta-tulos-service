...
class SijoittelunTulosRestClient(appConfig: AppConfig) {
  ...
  private def latestSijoitteluAjoUrl(hakuOid: String, hakukohdeOidOption: Option[String]): String = {
    val latestUrlForHaku = OphUrlProperties.ophProperties.url("sijoittelu-service.latest.url.for.haku", hakuOid)
  ...
  def fetchHakemuksenTulos(sijoitteluAjo: SijoitteluAjo, hakemusOid: String): Option[HakijaDTO] = {
    ...
    val url = OphUrlProperties.ophProperties.url("sijoittelu-service.hakemus.for.sijoittelu", hakuOid, sijoitteluAjo.getSijoitteluajoId, hakemusOid)
