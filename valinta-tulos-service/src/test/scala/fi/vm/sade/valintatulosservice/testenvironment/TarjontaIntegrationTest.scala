...
class TarjontaIntegrationTest extends Specification {
  "HakuService" should {
    "Extract response from tarjonta API"in {
      val haku = new TarjontaHakuService(null, AppConfig.fromString("IT")).getHaku("1.2.246.562.5.2013080813081926341927").right.get
  ...
  "HakuService fail case" should {
    "return Left for non existing haku ID" in {
      new TarjontaHakuService(null, AppConfig.fromString("IT")).getHaku("987654321").right.get must throwA[Throwable]
    }
  }
}
